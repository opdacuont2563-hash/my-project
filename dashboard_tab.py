...
def minutes_to_hhmm(minutes: float | int) -> str:
...
def load_bundle(kind: str) -> DataBundle:
    """โหลดข้อมูลจากฐาน Elective/Emergency ตามชนิดที่เลือก"""
    ...
    for label, db in targets:
        ...
        bundles.append(
            DataBundle(postop=postop, schedule=schedule, events=events, source_label=label)
        )
...
def clip_overlap_minutes(
    starts: pd.Series, ends: pd.Series, block_start_hhmm: str, block_end_hhmm: str
) -> pd.Series:
    """คำนวณนาทีที่ซ้อนทับกับ block เวลาทำการ"""
    ...
    os = pd.Series(
        np.maximum(starts.values.astype("datetime64[ns]"), bs.values.astype("datetime64[ns]"))
    )
    oe = pd.Series(
        np.minimum(ends.values.astype("datetime64[ns]"), be.values.astype("datetime64[ns]"))
    )
...
def enrich_cases(df_postop: pd.DataFrame, df_sched: pd.DataFrame) -> pd.DataFrame:
    ...
    if not df_sched.empty and {"date", "or_room", "doctor", "hn"}.issubset(df_sched.columns):
        m = df.merge(
            df_sched[
                ["date", "or_room", "doctor", "hn", "operation", "diagnosis"]
            ],
...
def trend(df: pd.DataFrame, block_start: str, block_end: str, granularity: str = "day") -> pd.DataFrame:
    ...
    room_day = (
        df.groupby(["bucket", "date"])["or_room"].nunique().rename("rooms").reset_index()
    )
...
def surgeon_first_cut(df: pd.DataFrame) -> pd.DataFrame:
    ...
    first["sec"] = (
        first["time_start_dt"].dt.hour * 3600
        + first["time_start_dt"].dt.minute * 60
        + first["time_start_dt"].dt.second
    )
    agg = first.groupby("doctor").agg(
        avg_first_sec=("sec", "mean"), workdays=("date", "nunique")
    ).reset_index()
...
class DashboardTab(QtWidgets.QWidget):
    ...
    def export_csv(self):
        ...
        path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Save CSV", "or_dashboard_export.csv", "CSV (*.csv)"
        )
        if path:
    ...
    def _plot_rooms(self, ru: pd.DataFrame):
        ...
        ax.set_ylabel("Util (%)")
        ax.set_ylim(0, 100)
        ax.set_title("Room Utilization (Ranking)")
        chart.canvas.draw()

    def _plot_firstcut(self, sf: pd.DataFrame):
        chart = self.firstCutChart
        chart.clear()
        ax = chart.ax
        if sf.empty:
            chart.canvas.draw()
    ...
    def _plot_topops(self, to: pd.DataFrame):
        ...
        ax.set_ylabel("Cases")
        ax.set_title("Top Operations")
        ax.tick_params(axis="x", rotation=30)
        chart.canvas.draw()

    def _plot_postpone(self, pp: pd.DataFrame):
        chart = self.postponeChart
        chart.clear()
        ax = chart.ax
        if pp.empty:
            chart.canvas.draw()
            return
        vals = pp["cases"].values
        labels = pp["reason"].astype(str).values
        ax.pie(vals, labels=labels, autopct="%1.0f%%", startangle=90)
        ax.axis("equal")
        ax.set_title("Postponement Reasons")
        chart.canvas.draw()

    def _plot_heatmap(self, hm: pd.DataFrame):
        chart = self.heatmapChart
        chart.clear()
        ax = chart.ax
        if hm.empty:
            chart.canvas.draw()
