...
class Main(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        here = Path(__file__).resolve().parent; lp = here / "MascotAlert.png"
        if lp.exists(): self.banner.setLogo(lp, size=34, radius=8)
        chipWrap = QtWidgets.QHBoxLayout(); chipWrap.setSpacing(6)
        holder = QtWidgets.QWidget(); holder.setLayout(chipWrap)
        self.status_chip = QtWidgets.QLabel("● Offline")
        self.status_chip.setStyleSheet("color:#ef4444;font-weight:800;padding:4px 8px;background:#fff;border:1px solid #e5e7eb;border-radius:999px;")
        self.btn_reconnect = ShadowButton("🔌 Reconnect", "#64748b"); self.btn_reconnect.clicked.connect(self._on_reconnect_clicked)
        self.btn_settings = ShadowButton("⚙️ Settings", "#0ea5e9"); self.btn_settings.clicked.connect(self._open_settings_dialog)
        chipWrap.addWidget(self.status_chip); chipWrap.addWidget(self.btn_reconnect); chipWrap.addWidget(self.btn_settings)
        self.banner.setRight(holder)
        root.addWidget(self.banner)

        # --- Top compact controls bar ---
        self.topControls = QtWidgets.QFrame()
        self.topControls.setStyleSheet("QFrame{background:#ffffff;border:1px solid #e6e6ef;border-radius:12px;}")
        tLay = QtWidgets.QHBoxLayout(self.topControls)
        tLay.setContentsMargins(8,8,8,8); tLay.setSpacing(8)

        # Identify
        card_ident = GlassCard("1. Identify Patient", "สแกนหรือกรอก HN • ใช้ Patient ID ได้", "🩺")
        g1 = card_ident.grid
        self.ent_hn = QtWidgets.QLineEdit(); self.ent_hn.setPlaceholderText("เช่น 590166994"); self.ent_hn.setMaxLength(9)
        self.ent_hn.setValidator(QtGui.QIntValidator(0, 999999999, self)); self.ent_hn.setFixedHeight(32); self.ent_hn.setMaximumWidth(200)
        self.ent_pid = QtWidgets.QLineEdit(); self.ent_pid.setFixedHeight(32); self.ent_pid.setMaximumWidth(220)
        g1.addWidget(QtWidgets.QLabel("HN (9 หลัก)"),0,0); g1.addWidget(self.ent_hn,0,1)
        g1.addWidget(QtWidgets.QLabel("Patient ID (ถ้ามี)"),1,0); g1.addWidget(self.ent_pid,1,1)
        scanRow = QtWidgets.QHBoxLayout(); self.chk_scan = QtWidgets.QCheckBox("โหมดสแกนบาร์โค้ด HN"); self.chk_scan.setChecked(True)
        self.chk_scan.stateChanged.connect(lambda s: setattr(self, "scan_enabled", bool(s)))
        self.lbl_scan_state = QtWidgets.QLabel("Scanner: Ready"); self.lbl_scan_state.setStyleSheet("color:#16a34a;font-weight:600;")
        scanRow.addWidget(self.chk_scan,0); scanRow.addStretch(1); scanRow.addWidget(self.lbl_scan_state,0,QtCore.Qt.AlignRight)
        g1.addLayout(scanRow,2,0,1,2)

        # Assign Room
        card_or = GlassCard("2. Assign Room", "กำหนด OR และ Queue", "🏥")
        g2 = card_or.grid
        self.cb_or = QtWidgets.QComboBox(); self.cb_or.addItems(OR_CHOICES); self.cb_or.setFixedWidth(110); self.cb_or.setFixedHeight(32)
        self.cb_q  = QtWidgets.QComboBox(); self.cb_q.addItems(QUEUE_CHOICES); self.cb_q.setFixedWidth(110); self.cb_q.setFixedHeight(32)
        g2.addWidget(QtWidgets.QLabel("OR"),0,0); g2.addWidget(self.cb_or,0,1)
        g2.addWidget(QtWidgets.QLabel("Queue"),1,0); g2.addWidget(self.cb_q,1,1)

        # Status & Timing
        card_stat = GlassCard("3. Status & Timing", "สถานะการผ่าตัดและเวลาโดยประมาณ", "⏱️")
        g3 = card_stat.grid
        self.cb_status = QtWidgets.QComboBox(); self.cb_status.addItems(STATUS_CHOICES); self.cb_status.setFixedWidth(200); self.cb_status.setFixedHeight(32)
        self.ent_eta = QtWidgets.QLineEdit(); self.ent_eta.setPlaceholderText("เช่น 90"); self.ent_eta.setFixedWidth(100); self.ent_eta.setFixedHeight(32)
        self.lbl_eta = QtWidgets.QLabel("เวลาโดยประมาณในการผ่าตัด (นาที)")
        g3.addWidget(QtWidgets.QLabel("Status"),0,0); g3.addWidget(self.cb_status,0,1)
        g3.addWidget(self.lbl_eta,1,0); g3.addWidget(self.ent_eta,1,1)
        self.cb_status.currentTextChanged.connect(self._toggle_eta_visibility); self._toggle_eta_visibility()

        # Action buttons (compact)
        action = Card("Action")
        wrap = QtWidgets.QFrame(); hl2 = QtWidgets.QHBoxLayout(wrap); hl2.setContentsMargins(0,0,0,0); hl2.setSpacing(0)
        def mk_btn(text, corner):
            btn = QtWidgets.QPushButton(text); btn.setCheckable(True)
        ...
        self.action_group = QtWidgets.QButtonGroup(self); self.action_group.setExclusive(True)
        ...
        self.btn_send = ShadowButton("🚀 ส่งคำสั่ง", "#10b981"); self.btn_send.setMinimumWidth(110); self.btn_send.clicked.connect(self._on_send)
        gridA = action.grid(); gridA.addWidget(wrap, 0, 0, 1, 2); gridA.addWidget(self.btn_send, 0, 2)

        for c in (card_ident, card_or, card_stat, action):
            c.setMaximumHeight(120)
            c.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed)
            tLay.addWidget(c)
        root.addWidget(self.topControls)
    ...
    def _on_health(self):
        ...
        def _extract_rows(self, payload):
            # แตก list ไอเท็มจาก payload ที่อาจมีหลายรูปแบบ
            src = []
            ...
            for i, it in enumerate(src, start=1):
                ...
                # --- Normalize สถานะให้เข้า STATUS_CHOICES ---
                status_raw = str(
                    it.get("status")
                    or it.get("state")
                ...
                # --- เวลา (timestamp): รองรับ epoch / iso / คีย์สำรอง ---
                ts_val = (
                        it.get("timestamp")
                        or it.get("ts")
